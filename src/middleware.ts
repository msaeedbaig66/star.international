...
function buildContentSecurityPolicy(nonce?: string) {
 ...
 `frame-src 'self' https://www.youtube.com https://www.youtube-nocookie.com https://*.google.com`,
...
function resolveApiRatePolicy(pathname: string, method: string): ApiRatePolicy {
 ...
 return { bucket: 'upload', limit: 6, windowMs: 60_000 } // Tightened to 6/min
 }
 if (pathname.startsWith('/api/listings') && method === 'POST') {
 return { bucket: 'listing-create', limit: 5, windowMs: 60_000 } // New protection
 }
 if (pathname.startsWith('/api/contact')) {
 return { bucket: 'contact', limit: 5, windowMs: 60_000 }
 }
 if (pathname.startsWith('/api/search')) {
 return { bucket: 'search', limit: 15, windowMs: 60_000 } // Tightened to 15/min
 }
 if (
 ...
 return { bucket: 'api-write', limit: 15, windowMs: 60_000 } // Tightened to 15/min
 }
 return { bucket: 'api-default', limit: 60, windowMs: 60_000 }
}
...
// ── Performance: pre-compute protected path prefixes ──
...
function isPublicRoute(pathname: string): boolean {
 // Public pages: no auth check needed at all
...
export async function middleware(request: NextRequest) {
 ...
 // ── Performance: API rate limiting path ──
 if (pathname.startsWith('/api/')) {
 const policy = resolveApiRatePolicy(pathname, request.method)
 ...
 // ── Performance: skip Supabase auth check for known public pages ──
 ...
 // Still refresh cookies if present, but don't block on getUser()
 ...
 // Refresh cookies if present, but don't block on getUser() for public routes
 // Only call getUser() if we have a session cookie to avoid unnecessary server load
 ...
 // Only check session in middleware to keep it fast
 ...
 // Verification check - still important to keep here to prevent unverified access to dashboards
 ...
 // Profile-based checks (onboarding, role) moved to Layout levels for better pipelining performance
 return applySecurityHeaders(supabaseResponse, nonce)
}
