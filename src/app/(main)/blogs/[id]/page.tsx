...
function getTocFromContent(content: string) {
 ...
 if (items.length >= 10) break; // Increased from 4 for better utility
 }
 if (items.length === 0) {
 return [
...
export default async function BlogDetailPage({ params }: { params: { id: string } }) {
 ...
 const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(routeKey);
 ...
 // View increment is now handled asynchronously by ViewTracker at the bottom of the page

 const [
 { data: relatedBlogs },
 { data: authorBlogs },
 { data: comments },
 { count: authorFollowersCount },
 { count: authorBlogsCount },
 userLikeRes,
 userFollowRes,
 userCommentLikesRes
 ] = await Promise.all([
 ...
 const authorFollowers = Number.isFinite(Number(authorFollowersCount))
 ? Number(authorFollowersCount)
 : Number(author?.follower_count || 0);
 const authorTotalBlogs = Number.isFinite(Number(authorBlogsCount))
 ? Number(authorBlogsCount)
