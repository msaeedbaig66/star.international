...
export async function POST(request: Request) {
  try {
    ...
    if (listing_id) {
      ...
      const { data: existingProfileRating, error: checkError } = await supabase
        .from('ratings')
        .select('id')
        .eq('reviewer_id', user.id)
    ...
    if (!aggregateReadError && subjectRatings) {
      ...
      // Update profile
      const { error: profileWriteError } = await supabase.from('profiles').update(profilePayload).eq('id', subject_id)
      if (profileWriteError && isPermissionDenied(profileWriteError)) {
        await getAdmin().from('profiles').update(profilePayload).eq('id', subject_id)
      }

      // 7. If rating was for a specific listing, recompute listing aggregates too
      if (listing_id) {
        const { data: listingRatings, error: listingReadError } = await supabase
          .from('ratings')
          .select('score')
          .eq('listing_id', listing_id)
        
        if (!listingReadError && listingRatings) {
          const lTotal = listingRatings.reduce((sum, r: any) => sum + Number(r.score || 0), 0)
          const lCount = listingRatings.length
          const lAverage = lCount > 0 ? Number((lTotal / lCount).toFixed(2)) : 0
          ...
          const { error: listingWriteError } = await supabase.from('listings').update(listingPayload).eq('id', listing_id)
          if (listingWriteError && isPermissionDenied(listingWriteError)) {
            await getAdmin().from('listings').update(listingPayload).eq('id', listing_id)
          }
        }
      }
    }
