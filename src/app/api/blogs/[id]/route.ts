...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    // Any author edit must return to moderation queue.
    updateData.moderation = 'pending'
    // Clear prior rejection/admin note when user resubmits updated content.
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    let fetchQuery = queryClient
      .from('blogs')
      .select('id, title, author_id, moderation, rejection_note')
      .eq('id', params.id)
    if (!canAdmin) {
    ...
    const note = encodeSoftDeleteNote(
      ...
      canAdmin ? 'Admin removed this blog. You can restore it within 2 days.' : 'You deleted this blog. You can restore it within 2 days.'
    )
    ...
    const { data, error } = await deleteQuery
      .select('id, title, moderation, rejection_note, updated_at')
      .single()
    ...
    if (canAdmin && existing.author_id !== user.id) {
      await admin!.from('notifications').insert({
        ...
        actor_id: user.id,
        blog_id: existing.id,
        message: `Admin deleted your blog "${existing.title}". You can undo this within 2 days.`,
        is_read: false,
      })
    }
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!meta) return NextResponse.json({ error: 'This blog is not recoverable.' }, { status: 400 })
    if (!isSoftDeleteRecoverable(meta)) {
      return NextResponse.json({ error: 'Recovery window expired (2 days).' }, { status: 400 })
    }

    const restoreModeration = meta.prevModeration || 'approved'
    let restoreQuery = queryClient
      ...
      .eq('id', params.id)
    if (!canAdmin) {
      restoreQuery = restoreQuery.eq('author_id', user.id)
    }
    ...
    if (canAdmin && existing.author_id !== user.id) {
      await admin!.from('notifications').insert({
        ...
        actor_id: user.id,
        blog_id: existing.id,
        message: `Admin restored your blog "${existing.title}".`,
        is_read: false,
      })
    }
