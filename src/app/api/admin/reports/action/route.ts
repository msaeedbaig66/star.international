...
import { deleteImagesByUrls, deleteImageByUrl } from '@/lib/cloudinary-server'
  ...
  try {
    ...
    if (action === 'undo') {
      ...
      if (!meta || meta.reportId !== reportId || !isUndoWindowOpen(meta)) {
        return NextResponse.json({ error: 'Undo window expired or action not found. Permanent deletions cannot be undone.' }, { status: 400 })
      }
    ...
    if (action === 'remove') {
      // PERMANENT DELETE as requested by user
      // 1. Cleanup Cloudinary images
      const imagesToDelete: string[] = []
      if (targetType === 'listing' || targetType === 'blog') {
        if (target.images) imagesToDelete.push(...target.images)
        if (target.cover_image) imagesToDelete.push(target.cover_image)
      } else if (targetType === 'community') {
        if (target.avatar_url) imagesToDelete.push(target.avatar_url)
        if (target.banner_url) imagesToDelete.push(target.banner_url)
      }
      ...
      // 2. Delete from DB
      const { error: deleteErr } = await supabase.from(table).delete().eq('id', target.id)
      ...
      if (ownerId) {
        await supabase.from('notifications').insert({
          user_id: ownerId,
          type: 'report_received', // General type for report-related notifications
          actor_id: user.id,
          message: adminMessage || `Admin has permanently removed your ${targetType} after report review.`,
          is_read: false,
        })
      }
