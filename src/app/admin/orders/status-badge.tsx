...
export function OrderActions({ orderId, currentStatus: initialStatus }: { orderId: string, currentStatus: string }) {
 ...
 return (
  ...
  {status === 'pending' && (
  <button onClick={() => updateStatus('approved')} disabled={loading} className="p-2 bg-primary/10 text-primary rounded-lg hover:bg-primary hover:text-white transition-all disabled:opacity-50">
  ...
  {status === 'approved' && (
  <button onClick={() => updateStatus('packed')} disabled={loading} className="p-2 bg-emerald-600/10 text-emerald-700 rounded-lg hover:bg-emerald-600 hover:text-white transition-all disabled:opacity-50">
  <span className={cn("material-symbols-outlined text-sm", loading && "animate-spin")}>
    {loading ? 'sync' : 'inventory_2'}
  </span>
  </button>
  )}

  {status === 'packed' && (
  <button onClick={() => setShowShipDialog(true)} disabled={loading} className="p-2 bg-emerald-500/10 text-emerald-600 rounded-lg hover:bg-emerald-500 hover:text-white transition-all disabled:opacity-50">
  <span className={cn("material-symbols-outlined text-sm", loading && "animate-spin")}>
    {loading ? 'sync' : 'local_shipping'}
  </span>
  </button>
  )}

  {status === 'shipped' && (
  <button onClick={() => updateStatus('delivered')} disabled={loading} className="p-2 bg-emerald-500/10 text-emerald-600 rounded-lg hover:bg-emerald-500 hover:text-white transition-all disabled:opacity-50">
  <span className={cn("material-symbols-outlined text-sm", loading && "animate-spin")}>
    {loading ? 'sync' : 'task_alt'}
  </span>
  </button>
  )}

  <button onClick={() => updateStatus('cancelled')} disabled={loading} className="p-2 bg-destructive/5 text-destructive rounded-lg hover:bg-destructive hover:text-white transition-all disabled:opacity-50">
  <span className={cn("material-symbols-outlined text-sm", loading && "animate-spin")}>
    {loading ? 'sync' : 'cancel'}
  </span>
  </button>
