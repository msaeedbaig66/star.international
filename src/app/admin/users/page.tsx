...
async function loadAllAuthUsers(supabase: ReturnType<typeof createAdminClient>) {
  ...
  const perPage = 1000 // Increase to maximum allowed
  const maxPages = 200  // Increase to support up to 200k users

  for (let page = 1; page <= maxPages; page += 1) {
    const { data, error } = await supabase.auth.admin.listUsers({ page, perPage })
    if (error) {
      console.error('Error fetching auth users batch:', error)
      break
    }
...
async function loadAllUsers() {
  ...
  // 1. Fetch all Auth Users (the ground truth for account existence)
  const authUsers = await loadAllAuthUsers(supabase)
  
  // 2. Fetch corresponding profiles for those IDs
