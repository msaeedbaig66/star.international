        ...
        <div className="flex-1 relative group/search">
          <form onSubmit={handleSearch} className="relative flex items-center">
            ...
            <input
              ...
              className="w-full bg-slate-100/60 dark:bg-zinc-900/60 border border-slate-200/50 dark:border-zinc-800/50 rounded-2xl py-3 pl-11 pr-14 focus:bg-white dark:focus:bg-zinc-950 focus:border-emerald-500/40 focus:ring-8 focus:ring-emerald-500/5 text-base md:text-[13px] font-bold text-slate-900 dark:text-zinc-100 outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-zinc-600 shadow-inner backdrop-blur-md"
            />
            <div className="absolute right-3 hidden lg:flex items-center gap-1 px-2 py-1 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-lg shadow-sm pointer-events-none opacity-40 group-focus-within/search:opacity-100 transition-opacity">
               <span className="text-[9px] font-black text-slate-400 dark:text-zinc-500">⌘</span>
               <span className="text-[9px] font-black text-slate-400 dark:text-zinc-500">K</span>
            </div>
          </form>
        </div>
      )}
        ...
        <div className="flex items-center gap-4 shrink-0">
          {!user ? (
            <Link 
              ...
              className="sm:hidden w-11 h-11 flex items-center justify-center rounded-xl bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 active:scale-95 transition-all"
            >
              <span className="material-symbols-outlined text-xl">login</span>
            </Link>
          ) : (
            <div className="flex items-center gap-4 pl-4 border-l border-slate-200/50 dark:border-zinc-800/50">
              {/* Notifications */}
              <div className="relative">
                <button 
                  onClick={() => { const willOpen = !notifOpen; setNotifOpen(willOpen); if (willOpen && !notifFetched) fetchNotifications(); }} 
                  className={cn(
                    "p-2.5 w-11 h-11 flex items-center justify-center text-slate-400 hover:text-emerald-500 rounded-xl hover:bg-slate-100 dark:hover:bg-zinc-900 relative transition-all active:scale-90", 
                    notifOpen && 'bg-emerald-50 text-emerald-500 dark:bg-emerald-500/10'
                  )}
                >
                  <span className="material-symbols-outlined text-2xl">notifications</span>
                  {unreadCount > 0 && (
                    <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-emerald-500 border-2 border-white dark:border-zinc-950 rounded-full animate-pulse shadow-sm" />
                  )}
                </button>
                
                {notifOpen && (
                  <div className="absolute right-0 mt-4 w-[calc(100vw-2rem)] sm:w-80 bg-white/95 dark:bg-zinc-950/95 border border-slate-200/50 dark:border-zinc-800/50 shadow-2xl rounded-3xl overflow-hidden py-3 z-[100] animate-reveal backdrop-blur-xl">
                    <div className="flex items-center justify-between px-5 pb-3 border-b border-slate-50 dark:border-zinc-900">
                    ...
                    <div className="max-h-80 overflow-y-auto px-3 py-2 flex flex-col gap-1.5 custom-scrollbar">
                      {!notifFetched ? (
                        ...
                        notifications.map(n => (
                          <div key={n.id} onClick={() => { !n.is_read && handleMarkAsRead(n.id); router.push(resolveNotificationLink(n)); setNotifOpen(false); }} className={cn("p-4 rounded-2xl cursor-pointer transition-all border group/item", !n.is_read ? 'bg-emerald-50/50 dark:bg-emerald-500/5 border-emerald-100/50 dark:border-emerald-500/20' : 'border-transparent hover:bg-slate-50 dark:hover:bg-zinc-900')}>
                            <div className="flex justify-between items-start mb-1">
                              <span className="font-black text-[10px] uppercase tracking-wider text-slate-900 dark:text-zinc-100 group-hover/item:text-emerald-500 transition-colors">{n.type}</span>
                              {!n.is_read && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-sm shadow-emerald-500/50" />}
                            </div>
                            <span className="text-xs text-slate-500 dark:text-zinc-400 line-clamp-2 leading-relaxed">{n.message}</span>
                          </div>
                        ))
                      )}
                    </div>
              ...
              {/* Profile */}
              <Link href={ROUTES.dashboard.home()} className="w-11 h-11 rounded-2xl overflow-hidden border-2 border-transparent hover:border-emerald-500/50 transition-all group shrink-0 shadow-lg shadow-black/5 active:scale-90 bg-slate-50 dark:bg-zinc-900">
                {profile?.avatar_url ? (
