...
function QuotaTracker({ label, used, limit, icon, isCapOnly }: { label: string; used: number; limit: number; icon: string; isCapOnly?: boolean }) {
 ...
 return (
 <div className="space-y-4 p-4 rounded-3xl bg-slate-50/50 border border-slate-200/40 group hover:border-primary/50 transition-all duration-300">
 <div className="flex items-center justify-between">
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded-xl bg-white flex items-center justify-center shadow-sm group-hover:bg-primary group-hover:text-white transition-all">
 ...
 {!isCapOnly ? (
 <span className="text-xs font-black text-slate-900 ">{used} <span className="text-slate-400 font-medium">/ {limit}</span></span>
 ) : (
 <span className="text-xs font-black text-slate-900 ">Max: {limit}</span>
 )}
 </div>
 </div>
 </div>
